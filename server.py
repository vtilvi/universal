...
def find_tithi_boundary(dt, target):
    ...
    for _ in range(30):
        mid = low + (high - low) / 2
        diff = (get_lon(astronomy.Body.Moon, mid) - get_lon(astronomy.Body.Sun, mid)) % 360
...
def compute_panchanga(date, time):
    ...
    nak_start = find_body_boundary(dt, astronomy.Body.Moon, nak_index * NAKSHATRA_SIZE)
    nak_end = find_body_boundary(dt, astronomy.Body.Moon, ((nak_index + 1) % 27) * NAKSHATRA_SIZE)
    moon_rashi_idx = int(moon_lon / 30)
    moon_rashi_start = find_body_boundary(dt, astronomy.Body.Moon, moon_rashi_idx * 30)
    moon_rashi_end = find_body_boundary(dt, astronomy.Body.Moon, ((moon_rashi_idx + 1) % 12) * 30)
    ...
    utc = dt - timedelta(hours=5, minutes=30)
    t = astronomy.Time.Parse(utc.strftime("%Y-%m-%dT%H:%M:%SZ"))
    planets = {names[i]: sidereal_longitude(bodies[i], t) for i in range(len(bodies))}
    return {
        ...
        "Moon_Rashi_start": moon_rashi_start,
        "Moon_Rashi_end": moon_rashi_end,
...
def world_calendar(request: Request):
    ...
    return templates.TemplateResponse(
        ...
        context={"result": compute_panchanga(d, t), "date": d, "time": t}
    )

@app.get("/calculate_json")
def calculate_json(date: str, time: str):
    return JSONResponse(compute_panchanga(date, time))

@app.get("/monitor", response_class=HTMLResponse)
def monitor(request: Request):
    return templates.TemplateResponse(request=request, name="worlddaynightmap.html")

@app.get("/solarsystem", response_class=HTMLResponse)
def solar_system(request: Request):
    return templates.TemplateResponse(request=request, name="solarsystem.html")

@app.get("/eclipse", response_class=HTMLResponse)
def eclipse_view(request: Request):
    return templates.TemplateResponse(request=request, name="eclipse.html")

# -------------------------------------------------
# AKSHAN TOOL
# -------------------------------------------------

@app.get("/akshan", response_class=HTMLResponse)
def akshan_view(request: Request):
    return templates.TemplateResponse(request=request, name="skbkg.html")
